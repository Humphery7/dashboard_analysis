...
def _demand_vs_price_plot(df_location: pd.DataFrame, location_name: str):
    ...
    fig.update_layout(
        title=dict(text=f"{location_name}: Top Products — Demand vs Price", x=0.5, font=dict(size=20)),
        template="plotly_white",
        xaxis_tickangle=-45,
        height=650,
...
def render_analytics() -> None:
    ...
    st.markdown(
        ...
        "Select a location to view Store 1 analysis. Store 2 remains a placeholder for now."
        "</p>",
    ...
    with left_col:
        st.subheader(f"Store 1 — {selected_location}")

        st.plotly_chart(_top_products_plot(selected_df, selected_location), use_container_width=True)
        _insight(f"This highlights the highest-demand products by total order count in {selected_location}.")
        st.markdown("<div style='height: 18px;'></div>", unsafe_allow_html=True)
